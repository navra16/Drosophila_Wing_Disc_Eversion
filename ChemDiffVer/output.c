	...
	const char *strng)
{
	...
	if (line == NULL)
	    FT_vector(&line,MAX_LINE_LEN+2,sizeof(char));
	...
	int	   reopen_stdin)
{
	...
	if ((currlist != nfiles) || (nfiles == NULL))
	{
	    ...
	    if (nf > 0)			
	        FT_vector(&olist,nf,sizeof(IOUTPUT));
	    for (i = 0; i < nf; ++i)
	...
	bool    all_nodes)
{
	    ...
	    ( (all_nodes == YES) || (io_node == YES)) )
	{
	    ...
	    if (alloc_len < strlen(dname))
	    {
	        ...
	        FT_vector(&dir,strlen(dname)+1,sizeof(char));
	        FT_vector(&partial_path,strlen(dname)+1,sizeof(char));
	        ...
	        if (next_path_component != NULL)
                {
                    ...
                    (void) sprintf(tmp,"%s/%s", partial_path,next_path_component);
                    (void) sprintf(partial_path,"%s", tmp);
