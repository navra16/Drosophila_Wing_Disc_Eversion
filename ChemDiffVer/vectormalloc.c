...
*				quin_array_T(),
*				sex_array_T(),
*	which allocate respectively (pointer to)scalar, vector, matrix
...
*
*	scalar(), vector(), matrix(), ...,  free_from()
*
*	Examples:
*
	 ...
	 ...)
{
	...
	if (order > 0)
	    ...
	    /* Allocate Space and memcpy pointers recursively: */
	...
	space = sizeof(ALIGN)*naligns;
        if(0 == space)
        {
            printf("ERROR: array_T() Allocating 0 bytes\n");
            clean_up(ERROR);
        }
