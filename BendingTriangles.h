...
// We want to take output forces on six nodes
struct CosBendingFunctor {
	...
	__host__ __device__ CosBendingFunctor(
		...
		double& _maxSpringScaler_bend,// Bending for Foppl-von Karman number? Unsure.
		double& _scaling_pow,
		double& _gausssigma,
	...
	// Hand in counting iterator and IDs of two triangles and two nodes involved. 
	__device__ double operator()(const Tuuuuu &u5) {
		...
		int id_l, id_j;
		// IDs of elements on the sides of a given edge
		// These are the same if we are at the edge of the membrane and those do not need to be computed. 
        ...
        // We need to compute rl and rj from the two involved triangles. 
				...
				// UD is the unit direction we use to check if the cross product is pointing in the right direction.
				...
				// While dUD_rj can be listed as zero vectors directly since it has no dependence on rj, it is written out fully for double-checking.
				...
				//DN1N2 := dot product dot(N1,N2), and "_rj" represents the partial derivative with respect to 1st, 2nd, and 3rd component of rj;
			...
			Mat_3x3	dcN1N2_rl = thrust::make_tuple<CVec3>(dcN1N2_rl_1, dcN1N2_rl_2, dcN1N2_rl_3);
				...
				// Apply Forces using counting iterator we place in the 4 spots of the x-y-z vectors for i,j,k,l
				// The Force must be reset since we are writing to a temp vector
				// Keys for reduced step must also be written.  
				if ((nN1 != 0.0) && (nN2 !=0.0)) {
					idKey[place] = id_i;
				...
				// Due to symmetry, it does not matter whether the angle is convex or concave. 
