...
void System::solveSystem(){
	...
	double current_edge_to_tip_height_scale = INT_MAX;//2.0;
	...
	double current_edge_to_tip_height_scale_ES = INT_MAX;//4.0;//2.0;
	...
	if (isRestiffening == true){
		scale_linear_restiff = linearSpringInfoVecs.spring_constant*1.0;//0.25;//25.0/2.5;//75.0/15.0;
		scale_bend_restiff = bendingTriangleInfoVecs.spring_constant*1.0;//0.05;//10.0/1.0;//75.0/7.5;
		scale_area_restiff = areaTriangleInfoVecs.spring_constant*1.0;//0.25;//50.0/5.0;//75.0/15.0;
	...
	// double u_scalingPower = 6;//8;//4;//6.0;
	...
	// 	v3 = coordInfoVecs.nodeLocZ[coordInfoVecs.triangles2Nodes_3[i]];
	// 	v4 = (v1+v2+v3)/3.0;
	// 	coordInfoVecs.u[i] = min_u + (max_u - min_u)*pow(((v4-min_cell_triangle_height)/cell_height),u_scalingPower);
	// }
	...
	double Max_Runtime = generalParams.dt*50.0;//50.0;
...
//	generalParams.edges_in_upperhem.resize(coordInfoVecs.num_edges);
	...
	while (runSim == true){
		...
		// 	coordInfoVecs.u[i] = min_u + (max_u - min_u)*pow((v4-min_cell_triangle_height)/cell_height, u_scalingPower);
		// }
		// coordInfoVecs.beta = 1.0/(min_u + ((/*cell_height - */2.0*generalParams.Rmin)/cell_height)*(max_u - min_u));
			...
			std::mt19937 generator_edgeswap(rand_dev());
				...
				for (int i = 0; i < coordInfoVecs.num_edges; i++){
					if (generalParams.edges_in_upperhem_list[i] >= 0 && 
						...
						if ((current_edge_to_tip_height) <= generalParams.Rmin*current_edge_to_tip_height_scale_ES && bdry_to_tip_height >= (generalParams.Rmin*bdry_to_tip_height_scale)){
							VectorShuffleForEdgeswapLoop.push_back(i);
						}
				...
				//if(bdry_to_tip_height > (generalParams.Rmin*bdry_to_tip_height_scale)){
				//	scale_bend = bendingTriangleInfoVecs.spring_constant*0.5;
				//	std::cout<<"Current bending stiffness changed to : "<<scale_bend<<std::endl;
				//	bendingTriangleInfoVecs.spring_constant_weak = scale_bend;
				//}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
		...
		// if (edgeswap_iteration >= round(NKBT/3.0)){
		// 	if (edgeswap_iteration == round(NKBT/3.0)){
		if (edgeswap_iteration >= round(NKBT*pointOfTransition)){
			...
			true_GROWTH_FREQUENCY = GROWTH_FREQUENCY2;
			generalParams.strain_threshold = strain_threshold2;
		}
		else{
			true_GROWTH_FREQUENCY = GROWTH_FREQUENCY;
		}
